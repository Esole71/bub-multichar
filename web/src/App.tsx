...
function App() {
	...
	const [uiConfig, setUiConfig] = useState<UIConfig>({
		...
		subtitle: "SELECT CHARACTER",
		defaultFivemName: "FiveM Name"
	});

	useNuiEvent<{ characters: Character[]; allowedCharacters: number; uiConfig: UIConfig }>(
		"showMultiChar",
		(data) => {
	...
	const HandleDelete = async (citizenid: string) => {
		...
		// Call the delete function with the correct parameter
		await fetchNui<number>(
			"deleteCharacter",
			{ citizenid: citizenid },
			{ data: 1 }
		...
		// No need to manually refresh - the server will send updated data
	};
												...
												<div className='character-card'>
													<Group justify='space-between'>
														...
														<Badge
															...
															radius='xs'
														>
															{character.fivemName || uiConfig.defaultFivemName}
														</Badge>
													</Group>
														...
														<SimpleGrid cols={2} spacing={3}>
															{character.metadata &&
																...
																character.metadata.map((metadata) => {
																	let label = metadata.value;
																	if (metadata.key === 'job') {
																		label = label.replace(/\s*\(0\)/, '');
																	}
																	return (
																		<InfoCard
																			key={metadata.key}
